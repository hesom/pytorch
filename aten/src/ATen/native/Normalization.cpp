...
TORCH_META_FUNC(native_batch_norm)(const at::Tensor & input,
  ...
  if (input.device().type() == c10::kCUDA){
    // This meta func matches the non-uniform dtype promotion done in CPU/CUDA impls.
    // It uses a larger type for stats accmulation on CUDA (half/bf16->float, float->double)
